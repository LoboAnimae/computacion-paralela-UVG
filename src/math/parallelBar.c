...
# pragma omp parallel num_threads(thread_count) shared(ld_current_error)
        {
            ...
            for (int j = omp_get_thread_num() * slice_width; j < omp_get_thread_num() * slice_width + slice_width; j++) {
                ...
                ld_current_error = (ld_current_error < fabsl(ldp_temperatures[j] - ld_new_temperature)) ? fabsl(
                        ldp_temperatures[j] - ld_new_temperature) : ld_current_error;
